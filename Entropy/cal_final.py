...
# 30분 간격 데이터를 CSV 파일로 저장
results.to_csv(f'{dir_path}/Results_{year}_{sheet_name}_30min.csv', index=False)
print(f"30-minute interval CSV file saved for {sheet_name}")

# 일 단위로 적산
daily_results = results.resample('D', on='Timestamp').sum()

# 일 단위 데이터를 CSV 파일로 저장
daily_results.to_csv(f'{dir_path}/Results_{year}_{sheet_name}_daily.csv')
print(f"Daily aggregated CSV file saved for {sheet_name}")